...
- **外部拥有账户（EOA）** 需要私钥签名才能发起交易，缺乏灵活的权限管理，一旦私钥丢失资产无法找回。
- **合约账户** 无法主动发起交易，必须由 EOA 调用，难以实现复杂的权限控制与自动化。

### 2. ERC-4337 的核心机制

- **UserOperation**：一种类似交易的数据结构，描述用户的操作意图、签名与费用等信息。
- **Bundler**：收集多笔 UserOperation，打包后通过 **EntryPoint** 合约一次性提交到链上。
- **EntryPoint 合约**：负责验证并执行操作，实现逻辑的统一入口。
- **Paymaster**：可为用户代付 Gas 或自定义计费策略。

### 3. 优势与实例

- **多重签名与自定义认证**：账户可内置多签逻辑或硬件/生物识别等多种验证方式。
- **Gas 代付**：通过 Paymaster 或第三方实现代付，用户无需持有原生代币即可与链交互。
- **社交恢复**：可设置可信联系人或多签流程恢复账户，降低私钥丢失风险。
- **实践案例**：StarkNet、zkSync 等 Layer2 网络已原生支持或通过 ERC-4337 引入 AA，为用户提供更友好的体验。
