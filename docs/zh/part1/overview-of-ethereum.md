...
2.  ==**EIP-4844（Cancun 升级）——“省钱神器”**=={.important}
    ...
    **EIP-4844 的突破**：
    ...
    - **Blob 数据结构**：Blob 是随区块一起传播的二进制大对象，每个约 128 KB，通过 KZG 承诺附加在区块上，EVM 无法直接读取
    - **临时存储机制**：节点仅需在约 1-3 个月内保存 Blob 数据，随后自动删除，避免长期占用主网存储

    **实际效果**：

    - **数据发布成本大降**：Rollup 将交易数据写入 Blob，费用与 calldata 分离且价格更低，大幅压缩数据发布支出
    - **吞吐量提升**：更便宜的数据空间让 Rollup 在同等成本下容纳更多交易，推动整体 TPS 上升
    - L2 交易费用降低 **70% - 90%**
