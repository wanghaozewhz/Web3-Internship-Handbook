...
Layer 2 是构建在主链之上的扩容技术，核心思想是**链下执行、链上结算**：大部分计算和存储在链下完成，仅将交易结果或证明提交到主链确认，从而节省区块空间并提高效率。

### 1. Rollup 方案

#### 乐观式 Rollup（Optimistic Rollup）

- **工作机制**：将多笔交易在链下打包并默认结果正确，定期把交易数据和状态根提交到主链。若有人质疑，可在挑战期内提交欺诈证明回滚。
- **优点**：实现相对简单，与现有 EVM 兼容性好，适合通用 Dapp。
- **缺点**：需要等待挑战期，提款速度慢，安全性依赖监控者。

#### 零知识 Rollup（ZK Rollup）

- **工作机制**：链下批量执行交易并生成零知识证明，主链只需验证证明即可更新状态，无需重新执行交易。
- **优点**：无需挑战期，确认快且安全性高，数据压缩效果好。
- **缺点**：生成证明计算复杂，开发门槛高，对 EVM 兼容性相对较弱。

### 2. 其他扩容方式

#### 状态通道
用户先在主链上锁定资金，在链下多次交互，最终一次性结算到主链。
- **降低 Gas**：链下交互无需支付手续费。
- **提高吞吐**：仅在开关通道时触链，链下交易几乎实时。

#### 侧链
独立运行的区块链，通过跨链桥与主链互联，资产可在侧链上流转。
- **降低 Gas**：侧链使用自己的共识和费率。
- **提高吞吐**：侧链可采用更高性能的参数配置。
