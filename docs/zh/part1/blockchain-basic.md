...
随着各类区块链生态蓬勃发展，资产与信息分散在不同网络，单链难以满足复杂的业务需求，跨链技术由此诞生，用以打通孤立的生态。

### 主流方案

- **资产交换**：通过原子交换等协议在不同链间直接兑换代币，无需中心化中介。
- **跨链桥**：在两条链之间锁定与铸造资产，实现代币跨链转移，常见如各类桥接服务。
- **数据互操作**：传递状态或消息，使智能合约能够跨链调用，例如轻客户端或 Relay 机制。

### 代表性项目

- **Polkadot**：采用中继链与平行链架构，提供共享安全的跨链通信。
- **Cosmos**：以 IBC 跨链通信协议连接独立区块链（Zone），实现生态互联。

### 技术挑战

- **原子性**：跨链交易需要确保两边操作要么同时成功要么同时失败，避免资产丢失。
- **安全验证**：如何在不完全信任对方链的情况下验证交易有效性，防范跨链桥被攻击。
